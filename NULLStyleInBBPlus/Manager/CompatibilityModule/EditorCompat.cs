...
namespace NULL.Manager.CompatibilityModule
{
    public static class EditorCompat
    {
        public static void Register() {
            ...
            {
                if (LevelStudioPlugin.Instance.npcDisplays.ContainsKey("NULL")) LevelStudioPlugin.Instance.npcDisplays.Remove("NULL");
                if (LevelStudioPlugin.Instance.npcDisplays.ContainsKey("NULLGLITCH")) LevelStudioPlugin.Instance.npcDisplays.Remove("NULLGLITCH");
                ...
                LevelStudioPlugin.Instance.StartCoroutine(DisableNullStyleInEditor());
            }

            if (LevelLoaderPlugin.Instance != null)
            {
                if (!LevelLoaderPlugin.Instance.npcAliases.ContainsKey("NULL")) LevelLoaderPlugin.Instance.npcAliases.Add("NULL", nullPrefab);
                if (!LevelLoaderPlugin.Instance.npcAliases.ContainsKey("NULLGLITCH")) LevelLoaderPlugin.Instance.npcAliases.Add("NULLGLITCH", glitchPrefab);
            }
        ...
        private static IEnumerator DisableNullStyleInEditor() {
            while (!LevelStudioPlugin.editorModesDefined) yield return null;
            ModManager.NullStyle = false;
        }
    }

    public class NullTool : NPCTool { public NullTool(Sprite sprite) : base("NULL", sprite) { } public override string titleKey => "NULL"; public override string descKey => "The main antagonist."; }
    public class NullGlitchTool : NPCTool { public NullGlitchTool(Sprite sprite) : base("NULLGLITCH", sprite) { } public override string titleKey => "NULL (Glitch)"; public override string descKey => "A glitchy variant."; }
}