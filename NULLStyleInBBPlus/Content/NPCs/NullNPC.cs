...
namespace NULL.NPCs {
    public class NullNPC : Baldi {
        ...
        void SetupPrefab() {
            ...
            navigator.passableObstacles.Add(PassableObstacle.BreakableWindow);
        ...
        public void FlickerLights(bool enable) {
            ...
            foreach (var cell in lightsToChange) {
                ...
                Vector3 diff = myPos - cell.TileTransform.position;
                ...
                if (!Singleton<PlayerFileManager>.Instance.reduceFlashing) {
                    if (sqrDist <= 900f) {
                        if (cell.lightOn) cell.SetLight(false);
                    }
                    else {
                        float actualDist = Mathf.Sqrt(sqrDist);
                        float num = (actualDist - 30f) / 70f;

                        if (Random.value <= 0.1f) {
                            if (!cell.lightOn)
                            {
                                if (Random.value <= num) cell.SetLight(true);
                            }
        ...
        public class SpeechCheck {
            ...
            public void Update() {
                ...
                if (speechCheckTime <= 0f) {
                    ...
                    if (!player.itm.HasItem() && player.plm.stamina == 0f &&
                        (nullNpc.transform.position - player.transform.position).sqrMagnitude <= 625f &&
                        (float)nullNpc.ReflectionGetVariable("anger") >= 4f)
    ...
    public class NullNPC_Chase : Baldi_Chase {
        ...
        public override void Enter() {
            ...
            if (!nullNpc.Navigator.passableObstacles.Contains(PassableObstacle.BreakableWindow))
                nullNpc.Navigator.passableObstacles.Add(PassableObstacle.BreakableWindow);
        }
        ...
        public override void OnStateTriggerStay(Entity otherEntity, Collider other, bool validCollision) {
            if (other.CompareTag("Player") && !nullNpc.Hidden) {
                Vector3 diff = nullNpc.transform.position - other.transform.position;
                bool flag;
                nullNpc.looker.Raycast(other.transform, diff.magnitude, out flag);
                if (flag) {
        ...
        public override void Update() {
            ...
            if (!(this is NullNPC_Preboss) && !(this is NullNPC_Rushing) && !nullNpc.Hidden) {
                if (!player.invincible) {
                    if ((nullNpc.transform.position - player.transform.position).sqrMagnitude < 25f) {
                        nullNpc.Speaker.SpeechChecker("Haha", 0.04f);
    ...
    public class NullNPC_Preboss : NullNPC_Chase {
        ...
        public NullNPC_Preboss(NullNPC nullNpc, Elevator finalElevator) : base(nullNpc, nullNpc) {
            this.targetElevator = finalElevator;
            this.targetPos = finalElevator.transform.position + finalElevator.transform.forward * 10f;
        }

        public override void Enter() {
            base.Enter();
            nullNpc.slideMode = true;
            nullNpc.baseSpeed = 60f;
            nullNpc.Navigator.SetSpeed(60f);
            nullNpc.Navigator.passableObstacles.Add(PassableObstacle.BreakableWindow);
            nullNpc.behaviorStateMachine.ChangeNavigationState(new NavigationState_TargetPosition(nullNpc, 100, targetPos));
        }

        public override void Update() {
            base.Update();
            if ((nullNpc.transform.position - targetPos).sqrMagnitude < 225f || !nullNpc.Navigator.HasDestination) {
                nullNpc.behaviorStateMachine.ChangeState(new NullNPC_Rushing(npc, nullNpc, targetElevator.transform.position));
            }
        }
    ...
    public class NullNPC_Rushing : NullNPC_Chase {
        ...
        public NullNPC_Rushing(NPC npc, NullNPC nullNPC, Vector3 elevatorPos) : base(npc, nullNPC) {
            this.finalPos = elevatorPos;
        }

        public override void Enter() {
            base.Enter();
            nullNpc.slideMode = true;
            nullNpc.baseSpeed = 80f;
            nullNpc.Navigator.SetSpeed(80f);
            nullNpc.Navigator.passableObstacles.Add(PassableObstacle.BreakableWindow);
            nullNpc.behaviorStateMachine.ChangeNavigationState(new NavigationState_TargetPosition(nullNpc, 100, finalPos));
        }

        public override void Update() {
            base.Update();
            if ((nullNpc.transform.position - finalPos).sqrMagnitude < 144f) {
