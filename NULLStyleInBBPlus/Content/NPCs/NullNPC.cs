...
public class NullNPC_Preboss : NullNPC_Chase {
    ...
    public override void Update() {
        base.Update();
        float dist = Vector3.Distance(nullNpc.transform.position, targetPos);
        if (dist < 15f || !nullNpc.Navigator.HasDestination) {
...
public class NullNPC_Rushing : NullNPC_Chase {
    ...
    public override void Update() {
        base.Update();
        float dist = Vector3.Distance(nullNpc.transform.position, finalPos);
        if (dist < 12f) {
            bool allReady = true;
            if (ModManager.DoubleTrouble) {
                foreach (var n in Object.FindObjectsOfType<NullNPC>()) {
                    if (n != nullNpc && Vector3.Distance(n.transform.position, finalPos) > 40f) {
                        allReady = false; 
                        break;
                    }
                }
            }
            if (allReady && !BossManager.Instance.BossActive && !BossManager.Instance.bossTransitionWaiting) {
